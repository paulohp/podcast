  ...
  DocPad = (function(_super) {
    ...
    */
    ...
    DocPad.prototype.generatePrepare = function(opts, next) {
      var config, docpad, generateStarted, locale, tasks, _ref1, _ref2;
      _ref1 = extractOptsAndCallback(opts, next), opts = _ref1[0], next = _ref1[1];
      docpad = this;
      config = docpad.getConfig();
      locale = docpad.getLocale();
      if ((_ref2 = opts.progress) != null) {
        _ref2.step("generate (preparing)").total(1).setTick(0);
      }
      ...
      tasks = new TaskGroup().on('item.run', function(item) {
        ...
        return (_ref3 = opts.progress) != null ? _ref3.step("generate (preparing: " + (item.getConfig().name) + ")").total(totals.total).setTick(totals.completed) : void 0;
      }).once('complete', next);
      if (opts.reset === true) {
        if (!docpad.hasPlugins()) {
          docpad.log('notice', locale.renderNoPlugins);
        }
        tasks.addTask('Check if the source directory exists', function(complete) {
        ...
        tasks.addTask('Clean our collections', function(complete) {
          return docpad.resetCollections(complete);
        });
        tasks.addTask('Populate our collections', function(complete) {
          return docpad.populateCollections(complete);
        });
        tasks.addTask('Add all database models to render queue', function() {
          return opts.collection != null ? opts.collection : opts.collection = new FilesCollection().add(docpad.getDatabase().models);
        });
      } else {
        tasks.addTask('Add only changed models to render queue', function() {
      ...
      tasks.addTask('generateBefore event', function(complete) {
    ...
    DocPad.prototype.generatePostpare = function(opts, next) {
      var collection, database, docpad, locale, server, _ref1, _ref2;
      _ref1 = extractOptsAndCallback(opts, next), opts = _ref1[0], next = _ref1[1];
      docpad = this;
      locale = docpad.getLocale();
      database = docpad.getDatabase();
      ...
      collection = opts.collection;
      if ((_ref2 = opts.progress) != null) {
        _ref2.step("generate (postparing)").total(1).setTick(0);
      }
